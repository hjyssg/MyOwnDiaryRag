...
| **日记批量总结** | 用本地 LM Studio 模型为**每一篇**日记写摘要，生成按年份排列的《日记总结.md》（输出到项目根目录 `output/`，运行期间叫「中途预览.md」） | [`batch_summary/README.md`](batch_summary/README.md) |
...
- **非法日期**：如 `2月30日`、`0230`、`02_30.txt`（2 月没有 30 日）——这种日期本来就不存在，多半是写错了。
- **月份错放**：如 `05月.txt` 里出现 `0912`（9 月的日记放进 5 月文件），或 `MM_DD.txt` 文件名月份与内容月份相差 2 个月以上。
- **年份错放**：文件名里内嵌的年份（如 `2025_股票日记.txt`）与所在年份文件夹不一致。

发现问题**不会阻止正常日记入库**，只在控制台用醒目区块重点提示，并在项目根目录生成
`日记导入日期检查报告.md` 警报报告（含文件名、行号与原始日期标记），供人工核对修正。
...
- 数据库文件（`*.db`）、`.env` 配置与批量总结产物（项目根目录 `output/`）都不会被提交到 Git

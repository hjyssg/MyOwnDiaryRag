...
产物只有项目根目录 `output/` 里的一个文件，就在仓库最外层（与 `scripts/`、`webapp-*/` 平级）：

| 阶段 | 文件 |
|---|---|
| 运行期间 | `output/中途预览.md`（每 60 秒刷新一次，随时打开看进度） |
| 跑完 | `output/日记总结.md` —— 由上面那个文件**改名**而来（标题 `# 日记总结` + 生成日期） |
| 中断 | 仍是 `output/中途预览.md`（只是快照，不算最终结果） |

每次运行覆盖同一个文件，不建时间戳子目录。
...
| `--rebuild-md` | 不调模型，用库里已有摘要重写「日记总结.md」 |
